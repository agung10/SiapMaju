				...
				<div class="row">
					<div class="col-md-12">
						<div class="form-group">
							...
							<input type="text" value="{{!empty($data->no_telp) ? $data->no_telp : '' }}" name="no_telp"
								class="form-control" />
						</div>
					</div>
					<div class="col-md-12">
						<div class="form-group">
							...
							<input type="text" value="{{!empty($data->mobile) ? $data->mobile : '' }}" name="mobile"
								class="form-control" />
						</div>
					</div>
					<div class="col-md-12">
						<div class="form-group">
							...
							<input type="text" value="{{!empty($data->nama_lokasi) ? $data->nama_lokasi : '' }}"
								name="nama_lokasi" class="form-control" />
						</div>
					</div>
					<div class="col-md-12">
						<div class="form-group">
							<label>Email <span class="text-danger">*</span></label>
							<input type="text" value="{{!empty($data->email) ? $data->email : '' }}" name="email"
								class="form-control" />
						</div>
					</div>
					<div class="col-md-12">
						<div class="form-group">
							...
							<input type="date" value="{{!empty($data->web) ? $data->web : '' }}" name="web"
								class="form-control" />
						</div>
					</div>
					<div class="col-md-12">
						<div class="form-group">
							...
							<input type="date" value="{{!empty($data->rekening) ? $data->rekening : '' }}"
								name="rekening" class="form-control" />
						</div>
					</div>
				</div>
					...
					<div class="col-md-12">
						<div class="form-group mb-1">
							...
							<textarea name="alamat" class="form-control" id="exampleTextarea"
								rows="3">{{!empty($data->alamat) ? $data->alamat : '' }}</textarea>
						</div>
					</div>
				</div>
			</div>
...
<script>
	const editData = async () => {
		...
		const res = async () => {
			return await fetch(`{{route('Beranda.Kontak.update','')}}/{{\Request::segment(3)}}`,{
		...
		if(status === 'success'){
			...
			localStorage.setItem('success','Kontak Berhasil Diupdate');
