			...
			<form id="headerForm" action="{{route('Kajian.Kategori.store')}}" enctype="multipart/form-data" method="POST">
				...
				<div class="card-body">
					...
					<div class="form-group">
						<label>Kategori Peraturan <span class="text-danger">*</span></label>
						<input type="text" name="kategori" class="form-control"  placeholder="Masukan Kategori Peraturan"/>
					</div>
				</div>
...
<script>
	...
	const storeData = async () => {
		...
		const res = async () => {
			return await fetch("{{route('Kajian.Kategori.store')}}",{
						headers:{
		...
		if(status === 'success'){
			...
			localStorage.setItem('success','Kategori Kajian Berhasil Ditambahkan');
