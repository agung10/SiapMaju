...
<div class="container">
    ...
    <div class="card card-custom gutter-b">
		<form id="headerForm" action="{{ route('UMKM.Kategori.update', $data->umkm_kategori_id) }}"  method="POST">
			@csrf
            {{ method_field('PATCH') }}
			<div class="card-body">
...
<script>
	...
	const storeData = async () => {
		...
		const res = await fetch(`{{route('UMKM.Kategori.update', '')}}/{{\Request::segment(3)}}`,{
			headers:{
		...
		if(status === 'success'){
			...
			localStorage.setItem('success','Kategori Produk Berhasil di Update');
