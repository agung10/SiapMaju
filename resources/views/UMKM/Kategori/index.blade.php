    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            <div class="card-toolbar">
                @include('partials.buttons.add', ['text' => 'Tambah Kategori Produk'])
            </div>
        </div>
        <div class="card-body">
...
<script>
    ...
    $(document).ready(function(){
        var table = $('#datatable').DataTable({
            ...
            serverSide: true,
            responsive:true,
            ajax: "{{ route('UMKM.Kategori.dataTables') }}",
            columns: [
    ...
    const deleteData = async (id) => {
           const res = await fetch(`{{ route('UMKM.Kategori.destroy','') }}/${id}`,{
    ...
    const deleteFunc = (id) => {
        swal.fire({
            title: "Konfirmasi",
            text: "Apakah anda yakin untuk menghapus Kategori Produk?",
            icon: "warning",
