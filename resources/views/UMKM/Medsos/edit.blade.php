...
<div class="container">
    ...
    <div class="card card-custom gutter-b">
        <form id="headerForm" action="{{ route('UMKM.Medsos.update', $data->medsos_id) }}" enctype="multipart/form-data"
            method="POST">
            @csrf
            {{ method_field('PATCH') }}
            <div class="card-body">
                <div class="form-group">
                    <div class="imageContainer">
                        <img class="imagePicture"
                            src="{{ ((!empty($data)) ? ((!empty($data->icon)) ? (asset('uploaded_files/medsos/'.$data->icon)) : (asset('images/NoPic.png'))) : (asset('images/NoPic.png'))) }}"
                    ...
                    <div class="custom-file">
                        ...
                        <label class="custom-file-label" for="customFile">{{ $data->icon }}</label>
                    </div>
                </div>
                <div class="form-group">
    ...
    <script>
		...
		const res = async () => {
			return  await fetch(`{{route('UMKM.Medsos.update','')}}/{{\Request::segment(3)}}`,{
		...
		if(status === 'success'){
			...
			localStorage.setItem('success','Medsos Sosial Berhasil Diupdate');
    ...
    const imageAgendaField = document.querySelector('input[name="icon"]')
