    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            <div class="card-toolbar">
                @include('partials.buttons.add', ['text' => 'Tambah Gallery Konten'])
            </div>
        </div>
        <div class="card-body">
...
<script>
    ...
    $(document).ready(function(){
        var table = $('#datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('Gallery.Content.dataTables', Route::current()->parameter('galeri_id')) }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex', className:'text-center', searchable: false , orderable: false},
                {data: 'file', name: 'galeri_konten.file', className:'text-center'},
                {data: 'keterangan', name: 'galeri_konten.keterangan'},
                {data: 'action', name: 'action', className:'text-center', searchable: false , orderable: false},
                {data: 'updated_at', name: 'galeri_konten.updated_at', searchable: false, visible: false },
            ],
            "order": [[ 4, "desc" ]],
    ...
    const deleteData = async (id) => {
        const res = await fetch(`{{ url('Gallery/'.Route::current()->parameter('galeri_id').'/GalleryContent') }}/${id}`,{
