...
@section('content')
    ...
    <div class="container">
        ...
        <div class="card card-custom gutter-b">
            <form class="form" id="form-pertanyaan-ubah" action="{{ route('Polling.Pertanyaan.update', ((!empty($data)) ? (\Crypt::encrypt($data->id_polling)) : (''))) }}" method="POST">
                {{ csrf_field() }}
                {{ method_field('PATCH') }}
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        <div class="col-lg-6">
                            ...
                            <input type="date" name="close_date" class="form-control" value="{{ $data->close_date }}" />
                        </div>
                    </div>
                    <hr>
                    <div class="form-group row">
                        <div class="col-lg-6">
                            ...
                            <textarea name="isi_pertanyaan" class="form-control" rows="5" placeholder="Masukkan Isi Pertanyaan">{{ $data->isi_pertanyaan }}</textarea>
                        </div>
                        <div class="col-lg-6 row-answer">
                                ...
                                @foreach ($data->answer as $key => $value)
                                    <div class="this-answer">
                                        <div class="input-answer">
                                            <input type="text" name="isi_pilih_jawaban[{{ $value->id_pilih_jawaban }}]" class="form-control choice-answer" value="{{ $value->isi_pilih_jawaban }}" {{ (($loop->last) ? ('') : ('readonly style=background-color:#80808038')) }}>
                                            <span class="btn btn-danger remove-choice" data-id="{{ $value->id_pilih_jawaban }}"><i class="fa fa-trash"></i></span>
                                        </div>
                            ...
                            @else
                                <div class="this-answer">
                                    <div class="input-answer">
                                        <input type="text" name="isi_pilih_jawaban[]" class="form-control choice-answer">
                                        <span class="btn btn-default remove-choice"><i class="fa fa-trash"></i></span>
                                    </div>
                                    <div class="err-answer" style="display:none;">Jawaban harus diisi !</div>
                                </div>
                            @endif
                            <span class="btn {{ ((count($data->answer) > 0) ? ('btn-info') : ('btn-default')) }} btn-add-answer">Tambah Jawaban</span>
                        </div>
                    </div>
                </div>
    ...
    {!! JsValidator::formRequest('App\Http\Requests\Polling\PertanyaanRequest', '#form-pertanyaan-ubah') !!}
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            const getDataLocation = (url, type, data, target = '', getContent = true) => {
                $.ajax({
                    url, type, data,
                    beforeSend: function() {
                    ...
                    error: function() {
                        Swal.fire('Error!', 'Maaf, Silakan refresh browser anda, telah terjadi kesalahan!', 'error');
                    }
                });
            };

            getDataLocation('{{ route('Polling.Pertanyaan.getCitiesByProvince') }}', 'POST', { 'id':'{{ $data->province_id }}', 'thisID':'{{ $data->city_id }}', '_token':'{{ csrf_token() }}' }, 'city_id', false);
            getDataLocation('{{ route('Polling.Pertanyaan.getSubdistrictsByCity') }}', 'POST', { 'id':'{{ $data->city_id }}', 'thisID':'{{ $data->subdistrict_id }}', '_token':'{{ csrf_token() }}' }, 'subdistrict_id', false);
            getDataLocation('{{ route('Polling.Pertanyaan.getWardsBySubdistrict') }}', 'POST', { 'id':'{{ $data->subdistrict_id }}', 'thisID':'{{ $data->kelurahan_id }}', '_token':'{{ csrf_token() }}' }, 'kelurahan_id', false);
            getDataLocation('{{ route('Polling.Pertanyaan.getRWsByWard') }}', 'POST', { 'id':'{{ $data->kelurahan_id }}', 'thisID':'{{ $data->rw_id }}', '_token':'{{ csrf_token() }}' }, 'rw_id', false);
            getDataLocation('{{ route('Polling.Pertanyaan.getRTsByRW') }}', 'POST', { 'id':'{{ $data->rw_id }}', 'thisID':'{{ $data->rt_id }}', '_token':'{{ csrf_token() }}' }, 'rt_id', false);
            ...
            $('body').on('change', 'select[name=province_id]', function() {
                ...
                if (_this.hasClass('btn-info')) {
                    ...
                    _clone.find('input').removeAttr('name value readonly style').attr('name', 'isi_pilih_jawaban[]').removeClass('is-valid').val('').end()
                        .find('.err-answer').css({'display':'none'});
                ...
                const _this = $(this);
                const answerID = _this.data('id');
                const _rowAnswer = _this.parents('.row-answer');

                $.ajax({
                    url: '{{ route('Polling.Pertanyaan.removeAnswer') }}',
                    type: 'POST',
                    data: { id:answerID, '_token':'{{ csrf_token() }}' },
                    beforeSend: function() {
                        $('#process-loading').css({'display':'block'});
                        $('div.container').css({'opacity':'0.2'});
                    },
                    success: function(result) {
                        ...
                        if (_rowAnswer.find('.this-answer').length <= 1) {
                            ...
                            _rowAnswer.find('span.btn-add-answer').removeClass('btn-info').addClass('btn-default').end()
                                .find('input').removeAttr('name value').attr('name', 'isi_pilih_jawaban[]').removeClass('is-valid');
                        }
                        else {
            ...
            }).on('submit', 'form#form-pertanyaan-ubah', function(e) {
                e.preventDefault();
                const _this = $(this);
