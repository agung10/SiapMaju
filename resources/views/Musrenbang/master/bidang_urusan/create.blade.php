...
<div class="container">
	...
	<div class="card card-custom gutter-b">
		<form id="headerForm" action="{{route('Musrenbang.Bidang-Urusan.store')}}" enctype="multipart/form-data" method="POST">
			@csrf
			<div class="card-body">
				<div class="row">
					<div class="col-md-12">
						<div class="form-group">
							...
							<input type="text" name="nama_bidang" class="form-control" placeholder="Masukan nama bidang" required />
						</div>
					</div>
				</div>
			</div>
...
<script>
	const storeData = async () => {
		...
		const res = async () => {
			return await fetch("{{route('Musrenbang.Bidang-Urusan.store')}}",{
				headers:{
		...
		if(status === 'success'){
			localStorage.setItem('success','Bidang Urusan Berhasil Ditambahkan');
