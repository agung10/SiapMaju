    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            <div class="card-toolbar">
                @include('partials.buttons.add', ['text' => 'Tambah Kategori Kegiatan'])
            </div>
        </div>
        <div class="card-body">
...
<script>
    ...
    $(document).ready(function(){
        var table = $('#datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('Master.KatKegiatan.dataTables') }}",
            columns: [
            {data: 'DT_RowIndex', name: 'DT_RowIndex', className:'text-center'},
            {data: 'nama_kat_kegiatan', name: 'nama_kat_kegiatan', className:'text-center'},
            {data: 'kode_kat', name: 'kode_kat', className:'text-center'},
    ...
    const deleteData = async (id) => {
           const res = await fetch(`{{ route('Master.KatKegiatan.destroy','') }}/${id}`,{
