...
<div class="container">
	...
	<div class="card card-custom gutter-b">
		<form id="dataForm" action="{{ route('Master.rw.update', $data->rw_id) }}" method="POST">
			@csrf
			{{ method_field('PATCH') }}
			<div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="text" value="{{ $data->rw }}" name="rw" class="form-control" />
                        </div>
                    </div>
                </div>
			</div>
...
{!! JsValidator::formRequest('App\Http\Requests\Master\RWRequest','#dataForm') !!}
<script>
	const editData = async () => {
        ...
        const res = async () => {
            return  await fetch(`{{route('Master.rw.update','')}}/{{\Request::segment(3)}}`,{
        ...
        if(status === 'success'){
            localStorage.setItem('success','RW Berhasil Diupdate');
            return location.replace('{{route("Master.rw.index")}}');
        }
