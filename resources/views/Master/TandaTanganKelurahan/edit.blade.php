...
<div class="container">
    ...
    <div class="card card-custom gutter-b">
        <form id="headerForm" action="{{ route('Master.TandaTanganKelurahan.update', $data->tanda_tangan_kelurahan_id) }}" enctype="multipart/form-data" method="POST">
            @csrf
            {{ method_field('PATCH') }}
            <div class="card-body">
                <div class="form-group">
                    <div class="imageContainer">
                        <img class="imagePicture" src="{{ ((!empty($data)) ? ((!empty($data->tanda_tangan_kelurahan)) ? (asset('uploaded_files/tanda_tangan_kelurahan/'.$data->tanda_tangan_kelurahan)) : (asset('images/NoPic.png'))) : (asset('images/NoPic.png'))) }}" width="200" height="200"></img>
                    </div>
                    <br>
                    <label>Tanda Tangan Kelurahan</label>
                    <div class="custom-file">
                        <input name="tanda_tangan_kelurahan" type="file" class="custom-file-input" id="customFile" />
                        <label class="custom-file-label" for="customFile">{{ $data->tanda_tangan_kelurahan }}</label>
                    </div>
                </div>
				<div class="row">
					<div class="col-md-6">
...
{!! JsValidator::formRequest('App\Http\Requests\Master\TandaTanganKelurahanRequest','#headerForm') !!}
<script>
    const editData = async () => {
		...
		const res = async () => {
			return  await fetch(`{{route('Master.TandaTanganKelurahan.update','')}}/{{\Request::segment(3)}}`,{
		...
		if(status === 'success'){
			localStorage.setItem('success','Tanda Tangan Kelurahan Berhasil Diupdate');
			return location.replace('{{route("Master.TandaTanganKelurahan.index")}}');
		}
    ...
    const imageAgendaField = document.querySelector('input[name="tanda_tangan_kelurahan"]')
