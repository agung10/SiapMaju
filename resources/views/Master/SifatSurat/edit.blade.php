		...
		<form id="dataForm"  method="POST">
			...
			<div class="card-body">
				<div class="form-group">
					...
					<input type="text" value="{{ !empty($data->sifat_surat) ? $data->sifat_surat : '' }}" name="sifat_surat" class="form-control" />
				</div>
			</div>
...
<script>
	...
	const storeData = async () => {
		...
		const res = await fetch(`{{route('Master.SifatSurat.update', '')}}/{{\Request::segment(3)}}`,{
						headers:{
		...
		if(status === 'success'){
			...
			localStorage.setItem('success','Sifat Surat Berhasil di Update');
