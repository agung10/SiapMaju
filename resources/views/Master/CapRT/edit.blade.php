...
<div class="container">
	...
	<div class="card card-custom gutter-b">
		<form id="dataForm" action="{{ route('Master.CapRT.update', $data->cap_rt_id) }}" enctype="multipart/form-data" method="POST">
			@csrf
			{{ method_field('PATCH') }}
            <div class="card-body">
                <div class="form-group">
                    <div class="imageContainer">
                        <img class="imagePicture" src="{{ ((!empty($data)) ? ((!empty($data->cap_rt)) ? (asset('uploaded_files/cap_rt/'.$data->cap_rt)) : (asset('images/NoPic.png'))) : (asset('images/NoPic.png'))) }}" width="200" height="200"></img>
                    </div>
                    <br>
                    <label>Cap RT</label>
                    <div class="custom-file">
                        <input name="cap_rt" type="file" class="custom-file-input" id="customFile" />
                        <label class="custom-file-label" for="customFile">{{ $data->cap_rt }}</label>
                    </div>
                </div>
				<div class="row">
					<div class="col-md-6">
...
<script>
    const editData = async () => {
		...
		const res = async () => {
			return  await fetch(`{{route('Master.CapRT.update','')}}/{{\Request::segment(3)}}`,{
		...
		if(status === 'success'){
			localStorage.setItem('success','Cap RT Berhasil Diupdate');
    ...
    const imageAgendaField = document.querySelector('input[name="cap_rt"]')
