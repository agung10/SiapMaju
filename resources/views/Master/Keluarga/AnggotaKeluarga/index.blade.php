    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            <div class="card-toolbar">
                @if($createBtnPermission)
                    @include('partials.buttons.add', ['text' => 'Tambah Anggota Keluarga'])
                    <a href="javascript:void(0)" class="btn btn-light-primary font-weight-bold addNewData d-none ml-5" target="_blank">
                        <i class="fas fa-plus-square"></i>
                        Tambah Data Anggota Keluarga Sesuai Pencarian
                    </a>
                @endif
            </div>
        </div>
...
<script>
    $(function() {
        ...
        function makeDataTable(province_id = '', city_id = '', subdistrict_id = '', kelurahan_id = '', rw_id = '', rt_id = ''){
            const route = '{{ route('Master.AnggotaKeluarga.dataTables') }}'
        ...
        $('.addNewData').click(function() {
            window.open("/Master/Keluarga/AnggotaKeluarga/create","_blank");
    ...
    const deleteData = async (id) => {
           const res = await fetch(`{{ route('Master.AnggotaKeluarga.destroy','') }}/${id}`,{
