...
<div class="container">
	...
	<div class="card card-custom gutter-b">
		<form id="dataForm" action="{{ route('Master.Tanda_Tangan_RT.update', $data->tanda_tangan_rt_id) }}" enctype="multipart/form-data" method="POST">
			@csrf
			{{ method_field('PATCH') }}
            <div class="card-body">
                <div class="form-group">
                    <div class="imageContainer">
                        <img class="imagePicture" src="{{ ((!empty($data)) ? ((!empty($data->tanda_tangan_rt)) ? (asset('uploaded_files/tanda_tangan_rt/'.$data->tanda_tangan_rt)) : (asset('images/NoPic.png'))) : (asset('images/NoPic.png'))) }}" width="200" height="200"></img>
                    </div>
                    <br>
                    <label>Tanda Tangan RT</label>
                    <div class="custom-file">
                        <input name="tanda_tangan_rt" type="file" class="custom-file-input" id="customFile" />
                        <label class="custom-file-label" for="customFile">{{ $data->tanda_tangan_rt }}</label>
                    </div>
                </div>
				<div class="row">
					<div class="col-md-6">
...
{!! JsValidator::formRequest('App\Http\Requests\Master\TandaTanganRTRequest','#dataForm') !!}
<script>
    const editData = async () => {
		...
		const res = async () => {
			return  await fetch(`{{route('Master.Tanda_Tangan_RT.update','')}}/{{\Request::segment(3)}}`,{
		...
		if(status === 'success'){
			localStorage.setItem('success','Tanda Tangan RT Berhasil Diupdate');
			return location.replace('{{route("Master.Tanda_Tangan_RT.index")}}');
		}
    ...
    const imageAgendaField = document.querySelector('input[name="tanda_tangan_rt"]')
