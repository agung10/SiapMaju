    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            <div class="card-toolbar">
                @include('partials.buttons.add', ['text' => 'Tambah Transaksi'])
            </div>
        </div>
        <div class="card-body">
...
<script>
    ...
    $(document).ready(function(){
        var table = $('#datatable').DataTable({
            ...
            serverSide: true,
            responsive:true,
            ajax: "{{ route('Master.Transaksi.dataTables') }}",
            columns: [
            {data: 'DT_RowIndex', name: 'DT_RowIndex', className:'text-center'},
            {data: 'nama_transaksi', name: 'nama_transaksi', className:'text-center'},
    ...
    const deleteData = async (id) => {
           const res = await fetch(`{{ route('Master.Transaksi.destroy','') }}/${id}`,{
