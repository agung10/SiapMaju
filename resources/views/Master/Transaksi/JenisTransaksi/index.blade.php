    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            <div class="card-toolbar">
                @include('partials.buttons.add', ['text' => 'Tambah Jenis Transaksi'])
            </div>
        </div>
        <div class="card-body">
...
<script>
    ...
    $(document).ready(function(){
        var table = $('#datatable').DataTable({
            ...
            serverSide: true,
            responsive:true,
            ajax: "{{ route('Master.JenisTransaksi.dataTables') }}",
            columns: [
            {data: 'DT_RowIndex', name: 'DT_RowIndex', className:'text-center'},
            {data: 'nama_kegiatan', name: 'nama_kegiatan', className:'text-center'},
            {data: 'nama_jenis_transaksi', name: 'nama_jenis_transaksi', className:'text-center'},
    ...
    const deleteData = async (id) => {
           const res = await fetch(`{{ route('Master.JenisTransaksi.destroy','') }}/${id}`,{
