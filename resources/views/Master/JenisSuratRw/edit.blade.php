		...
		<form id="dataForm"  method="POST">
			...
			<div class="card-body">
				<div class="form-group">
					...
					<input type="text" value="{{ !empty($data->jenis_surat) ? $data->jenis_surat : '' }}" name="jenis_surat" class="form-control" />
				</div>
			</div>
...
<script>
	...
	const storeData = async () => {
		...
		const res = await fetch(`{{route('Master.JenisSuratRw.update', '')}}/{{\Request::segment(3)}}`,{
						headers:{
		...
		if(status === 'success'){
			...
			localStorage.setItem('success','Jenis Surat RW Berhasil di Update');
