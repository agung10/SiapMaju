...
<div class="container">
	...
	<div class="card card-custom gutter-b">
		<form id="dataForm" action="{{ route('Master.TandaTanganRW.update', $data->tanda_tangan_rw_id) }}" enctype="multipart/form-data" method="POST">
			@csrf
			{{ method_field('PATCH') }}
            <div class="card-body">
                <div class="form-group">
                    <div class="imageContainer">
                        <img class="imagePicture" src="{{ ((!empty($data)) ? ((!empty($data->tanda_tangan_rw)) ? (asset('uploaded_files/tanda_tangan_rw/'.$data->tanda_tangan_rw)) : (asset('images/NoPic.png'))) : (asset('images/NoPic.png'))) }}" width="200" height="200"></img>
                    </div>
                    <br>
                    <label>Tanda Tangan RW</label>
                    <div class="custom-file">
                        <input name="tanda_tangan_rw" type="file" class="custom-file-input" id="customFile" />
                        <label class="custom-file-label" for="customFile">{{ $data->tanda_tangan_rw }}</label>
                    </div>
                </div>
				<div class="row">
					<div class="col-md-6">
...
{!! JsValidator::formRequest('App\Http\Requests\Master\TandaTanganRWRequest','#dataForm') !!}
<script>
    const editData = async () => {
		...
		const res = async () => {
			return  await fetch(`{{route('Master.TandaTanganRW.update','')}}/{{\Request::segment(3)}}`,{
		...
		if(status === 'success'){
			localStorage.setItem('success','Tanda Tangan RW Berhasil Diupdate');
			return location.replace('{{route("Master.TandaTanganRW.index")}}');
		}
    ...
    const imageAgendaField = document.querySelector('input[name="tanda_tangan_rw"]')
