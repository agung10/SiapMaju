...
<div class="container">
	...
	<form class="form">
		<div class="card-body">
			...
			<div class="form-group row">
				<div class="col-lg-12">
					...
					<div class="form-group row">
						<div class="col-lg-2">
							<input disabled type="text" value="{{$noBukti}}" class="form-control buktiNo" placeholder="No"/>
						</div>
						<div class="col-lg-2">
			...
			<div class="form-group row">
				<div class="col-lg-9">
					...
					<div class="form-group row">
						...
						<div class="col-lg-3">
							<input type="text" disabled value="{{date('m-Y',strtotime($data->tgl_pendaftaran))}}" class="form-control headerInput monthPendaftaran" placeholder="Pilih Bulan"/>
							<span class="errorHeader"></span>
						</div>
					</div>
				</div>
				<div class="col-lg-3">
					...
					<input type="text" disabled value="{{date('d-m-Y',strtotime($data->tgl_pendaftaran))}}" class="form-control headerInput" placeholder="Pilih Bulan"/>
				</div>
			</div>
			<div class="form-group row">
				<div class="col-lg-6">
		...
		@if(!empty($data->no_bukti))
			<div class="col-lg-12 text-right printButtonContainer">
				<button type="submit" class="btn btn-primary mr-5 printButton">Print</button>
			</div>
		@endif
	</form>
...
<script>
	document.querySelector('.printButton')?.addEventListener('click',e => {
		e.preventDefault();	
		// const url = `{{route('Transaksi.Approval.edit',':id')}}`.replace(':id','{{\Crypt::encryptString($data->header_trx_kegiatan_id)}}')
		const url = `{{route('Transaksi.Header.create_pdf',':id')}}`.replace(':id','{{\Crypt::encryptString($data->header_trx_kegiatan_id)}}')
