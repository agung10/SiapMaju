...
<script>
          ...
          .then(async (result) => {
                if (result.isConfirmed) {
                    if(detail_trx_kegiatan_id){
						const res = await fetch(`{{route('Transaksi.Header.deleteDetail','')}}/${detail_trx_kegiatan_id}`,{
													headers:{
														'X-CSRF-TOKEN':'{{csrf_token()}}',
														'Content-Type':'application/json',
														'X-Requested-With':'XMLHttpRequest'
	...
	document.querySelector('.sendTransaksi').addEventListener('click',async ()=>{
		...
		if(checkField){
			const res = await fetch(`{{route('Transaksi.Header.updateDetail','')}}/${header_trx_kegiatan_id}`,{
