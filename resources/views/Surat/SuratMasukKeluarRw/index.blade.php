    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            <div class="card-toolbar">
                @include('partials.buttons.add', ['text' => 'Buat Surat Masuk Dan Keluar RW'])
            </div>
        </div>
        <div class="card-body">
...
<script>
    ...
    $(document).ready(function(){
        var table = $('#datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('Surat.SuratMasukKeluarRw.dataTables') }}",
            columns: [
            ...
            {data: 'tgl_surat', name: 'tgl_surat', className:'text-center'},
    ...
    const deleteData = async (id) => {
           const res = await fetch(`{{ route('Surat.SuratMasukKeluarRw.destroy','') }}/${id}`,{
           ...
           if(status === 'success'){
             swal.fire("Success", "Surat Masuk dan Keluar Berhasil Di Hapus!", "success");
    ...
    const deleteFunc = (id) => {
        swal.fire({
            title: "Konfirmasi",
            text: "Apakah anda yakin untuk menghapus Surat Masuk dan Keluar ini?",
            icon: "warning",
