    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            <div class="card-toolbar">
                @include('partials.buttons.add', ['text' => 'Tambah Kategori Laporan'])
            </div>
        </div>
        <div class="card-body">
...
<script>
    ...
    $(document).ready(function(){
        var table = $('#datatable').DataTable({
            ...
            serverSide: true,
            responsive:true,
            ajax: "{{ route('Laporan.KategoriLaporan.dataTables') }}",
            columns: [
    ...
    const deleteData = async (id) => {
           const res = await fetch(`{{ route('Laporan.KategoriLaporan.destroy','') }}/${id}`,{
