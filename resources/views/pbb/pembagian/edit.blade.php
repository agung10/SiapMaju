...
<div class="container">
    ...
    <div class="card card-custom gutter-b">
		<form id="form" action="{{ route('pbb.pembagian.update', $pbb->encryptedId) }}" enctype="multipart/form-data" method="POST">
			@csrf
            {{ method_field('PATCH') }}
			<div class="card-body">
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							...
							<select name="blok_id" class="form-control">
							  <option></option>
							    @foreach($blok as $b)
							        <option value="{{ $b->blok_id }}" 
							        	data-nop="{{ $b->nop }}" 
							        	{{ $pbb->blok_id == $b->blok_id ? 'selected' : '' }}
							        > 
							          {{ $b->nama_blok }}
							        </option>
							    @endforeach
						...
						<div class="form-group">
							...
							@include('partials.form-select', [
		                      ...
		                      'data'        => $warga,
		                      'selected'    => $pbb->anggota_keluarga_id
		                    ])
						</div>
						<div>
							...
							<div class="form-group">
								<div class="custom-file">
									...
									<label class="custom-file-label" for="customFile" id="file-name">{{ $pbb->foto_terima ? $pbb->foto_terima : 'Choose file' }} </label>
								</div>
							</div>
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<input type="text" name="nop" value="{{ $pbb->nop }}" class="form-control" />
						</div>

						<div class="form-group">
							<label>Tanggal Terima <span class="text-danger">*</span></label>
							<div class="input-group">
								<input name="tgl_terima" readonly type="text" class="form-control" value="{{ date('d-m-Y', strtotime($pbb->tgl_terima)) }}" placeholder="Pilih Tanggal"/>
						...
						<div class="form-group">
							...
							<div class="input-group">
								<input name="tahun_pajak" readonly type="text" class="form-control" value="{{ $pbb->tahun_pajak }}" placeholder="Pilih Tanggal"/>
								<div class="input-group-append">
									<span class="input-group-text">
										<i class="la la-calendar icon-lg"></i>
									</span>
